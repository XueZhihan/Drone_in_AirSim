...
parser.add_argument('--epoch', default=1, type=int)  # buffer采样的数据训练几次
...
class PolicyNetwork(nn.Module):
    ...
    def forward(self, state, deterministic=False):
        ...
        log_prob = log_prob.sum((-1, 0), keepdim=True)
...
class SAC(object):
    ...
    def __init__(self, env,  state_dim, action_dim, seed=0, hidden_dim=[256, 128],
                 ...
                 tau=0.005, lr=1e-4, auto_alpha=True, batch_size=100, start_steps=10000,
    ...
    def  select_action(self, state):
        ...
        z, mean, log_std, log_prob, std = self.policy_net(state)


        action = z.detach().cpu().numpy()
...
def main():
    ...
    if args.mode == 'test':
        ...
        if args.render: env.render(directory)
        print("====================================")
        ...
        for i in range(args.num_episode):
            ...
            for t in range(args.max_frame):
                ...
                agent.memory.append(state, action, reward, next_state, done, info)
                agent.num_transition += 1
                if agent.num_transition >= args.capacity and t % 5 == 0:
                    agent.update(args.epoch, args.batch_size)
