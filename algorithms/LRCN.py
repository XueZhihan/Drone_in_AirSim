...
class LRCN(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, num_layers, bidirectional, num_classes, device):
    ...
    def _init_hidden(self, batch_size):
        h = torch.zeros(self.num_layers * self.num_dirs, batch_size, self.hidden_dim).to(self.device)  # h0
        c = torch.zeros(self.num_layers * self.num_dirs, batch_size, self.hidden_dim).to(self.device)  # c0
        return (h, c)
