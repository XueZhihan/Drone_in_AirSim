...
class R2Plus1DNet(nn.Module):
    ...
    def forward(self, x):
        x = self.conv1(x)  # L*112*112 --> L*56*56
        x = self.conv2(x)  # L*56*56 --> L*56*56
        x = self.conv3(x)  # L*56*56 --> (L/2)*28*28
        x = self.conv4(x)  # (L/2)*28*28 --> (L/4)*14*14
        x = self.conv5(x)  # (L/4)*14*14 --> (L/8)*7*7

        x = self.pool(x)  # (L/8)*7*7  --> 1*1*1

        return x.view(-1, 512)  # (B, 512)
...
if __name__ == "__main__":
    ...
    R21Dmodel = R2Plus1DClassifier(101, (2, 2, 2, 2))  # ResNet -18
    outputs = R21Dmodel(inputs)
