...
class SAC():
    ...
    def update(self):
        ...
        s = torch.FloatTensor([t.s for t in self.replay_buffer]).float().to(device)
        a = torch.FloatTensor([t.a for t in self.replay_buffer]).to(device)
        r = torch.FloatTensor([t.r for t in self.replay_buffer]).to(device)
        s_ = torch.FloatTensor([t.s_ for t in self.replay_buffer]).float().to(device)
        d = torch.FloatTensor([t.d for t in self.replay_buffer]).float().to(device)
        ...
        for _ in range(args.epoch):
            ...
            pi_loss = (log_prob - excepted_new_Q).detach().mean().requires_grad_()  # according to original paper
